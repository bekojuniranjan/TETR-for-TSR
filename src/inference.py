...
def structure_to_cells(table_structure, tokens, img):
    """
    ...
    for cell, cell_span_nums in zip(cells, span_nums_by_cell):
        ...
        cell_pil_image = img.crop(cell['bbox'])
        cell['cell text'] = pytess(cell_pil_img=cell_pil_image)
...
class TableExtractionPipeline(object):
    ...
    def recognize(self, img, tokens=None, out_objects=False, out_cells=False,
        ...
        tables_cells = [structure_to_cells(structure, tokens, img)[0] for structure in tables_structure]
        if out_cells:
